...
from nnunetv2.custom_networks.nnformer.utilities.nd_softmax import softmax_helper
from torch import nn
import torch
import numpy as np
from nnunetv2.custom_networks.nnformer.initialization import InitWeights_He
from nnunetv2.custom_networks.nnformer.neural_network import SegmentationNetwork
...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        if self.deep_supervision: # do not support deep_supervision at all scales
...
if __name__ == "__main__":
    i = torch.tensor([2, 4, 96, 96, 96]).cuda()
    net = nnFormer(
        crop_size=[96, 96, 96], input_channels=4, num_classes=2, deep_supervision=True
    ).cuda()
    print(net)
    o = net(i)
    print(o)
