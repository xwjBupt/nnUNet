...
class SegMamba(nn.Module):
    """SegMamba wrapper compatible with nnU-Net v2 plans-based construction.

    nnU-Net v2 creates the network with keyword arguments named
    ``input_channels`` and ``num_classes``. The original SegMamba implementation
    used ``in_chans`` and ``out_chans``. This constructor accepts both forms, so
    it can be used directly from ``nnUNetPlans.json -> architecture``.
    """
    ...
    def forward(self, x_in):
        ...
        # nnU-Net v2 may pass deep_supervision=True to the architecture.
