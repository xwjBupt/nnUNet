...
def copy_jsons_to_output_dir(model_training_output_dir):
    root_dir = os.path.dirname(model_training_output_dir)
    jsons = glob.glob(root_dir + "/*.json")
    for json in jsons:
...
def predict_from_raw_data(
    ...
    if configuration_manager.previous_stage_name is not None:
        if folder_with_segs_from_prev_stage is None:
            ...
            assert num_parts == 1 and part_id == 0, (
                "folder_with_segs_from_prev_stage was not given and inference "
    ...
    if (
        ...
        and (len(list_of_lists_or_source_folder) > 5)
    ):  # just a dumb heurisitic in order to skip compiling for few inference cases
...
def predict_entry_point():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/home/xwj/WORK/code/nnUNet/nnUNet_results/Dataset517_DRIVE/nnUNetTrainer__nnUNetPlans__2d/9_18#9_29@ERNet_E7_D3-D2_Drop0_1500e",
        help="Output folder. If it does not exist it will be created. Predicted segmentations will "
        "have the same name as their source images.",
    )
    parser.add_argument(
