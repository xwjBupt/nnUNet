...
class Conv3d_Block(nn.Module):
    def __init__(
        self, num_in, num_out, kernel_size=1, stride=1, g=1, padding=None, norm=None
    ):
...
class MFunit(nn.Module):
    def __init__(self, num_in, num_out, g=1, stride=1, d=(1, 1), norm=None):
        ...
        self.conv3x3x3_m1 = DilatedConv3DBlock(
            num_mid,
            num_out,
            kernel_size=(3, 3, 3),
            stride=stride,
        ...
        self.conv3x3x3_m2 = DilatedConv3DBlock(
            num_out,
            num_out,
            kernel_size=(3, 3, 1),
            stride=1,
...
class DMFUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.conv1x1x1_in2(x1)
        x3 = (
            self.weight1 * self.conv3x3x3_m1[0](x2)
            + self.weight2 * self.conv3x3x3_m1[1](x2)
            + self.weight3 * self.conv3x3x3_m1[2](x2)
        )
        x4 = self.conv3x3x3_m2(x3)
        shortcut = x
        if hasattr(self, "conv1x1x1_shortcut"):
...
class MFNet(nn.Module):  #
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm="bn", num_classes=4):
        ...
        self.seg = nn.Conv3d(
            n, num_classes, kernel_size=1, padding=0, stride=1, bias=False
        )
...
class DMFNet(MFNet):  # softmax
    ...
    def __init__(self, c=1, n=32, channels=128, groups=16, norm="bn", num_classes=2):
        ...
        self.encoder_block2 = nn.Sequential(
            ...
            DMFUnit(
                channels, channels, g=groups, stride=1, norm=norm, dilation=[1, 2, 3]
            ),  # Dilated Conv 3
            DMFUnit(
                channels, channels, g=groups, stride=1, norm=norm, dilation=[1, 2, 3]
            ),
