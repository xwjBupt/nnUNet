...
from nnunetv2.custom_networks.PHTrans.swin_3D import *
...
class ConvDropoutNormNonlin(nn.Module):
    def __init__(
        ...
        self.conv = conv_op(int(input_channels), int(output_channels), **conv_kwargs)
        ...
        if (
            ...
            self.dropout = None
        self.instnorm = norm_op(int(output_channels), **norm_op_kwargs)
...
class DownOrUpSample(nn.Module):
    def __init__(
        ...
        nonlin=nn.LeakyReLU,
        nonlin_kwargs=None,
        nonlinbasic_block=ConvDropoutNormNonlin,
    ):
        super(DownOrUpSample, self).__init__()
        self.blocks = nonlinbasic_block(
            input_feature_channels,
            output_feature_channels,
            conv_op,
            conv_kwargs,
...
class BasicLayer(nn.Module):
    def __init__(
        ...
        dim = int(
            min(
                (base_num_features * feat_map_mul_on_downscale**num_stage),
                max_num_features,
            )
        )
        ...
        self.input_du_channels = int(dim)
        self.output_du_channels = int(
            min(
                int(
                    base_num_features
        ...
        if down_or_upsample is not None:
            ...
            self.down_or_upsample = nn.Sequential(
                down_or_upsample(
                    self.input_du_channels,
                    self.output_du_channels,
