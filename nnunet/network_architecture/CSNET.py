...
class SpatialAttentionBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.query = nn.Conv2d(
            in_channels, in_channels // 8, kernel_size=(1, 3), padding=(0, 1)
        )
        self.key = nn.Conv2d(
            in_channels, in_channels // 8, kernel_size=(3, 1), padding=(1, 0)
        )
...
class CSNet(nn.Module):
    def __init__(
        self, in_ch=1, out_ch=1, weightInitializer=InitWeights_He(1e-2), **kwargs
    ):
        """
        ...
        self.enc_input = ResEncoder(in_ch, 32)
...
class CSNET(SegmentationNetwork):
    def __init__(
        ...
        self.model = CSNet(in_ch=init_ch, out_ch=num_classes)
        ######## self.model 设置自定义网络 by Sleeep ########

        self.name = "CSNET_RES101"
...
if __name__ == "__main__":
    model = CSNET(num_classes=1, init_ch=3)
    dum = torch.rand([1, 3, 640, 640])
    print(model)
    out = model(dum)
    print(out.shape)
