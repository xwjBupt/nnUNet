...
class nnUNetTrainer(NetworkTrainer):
    def __init__(
        ...
        self.online_eval_foreground_sen = []
        self.online_eval_foreground_spe = []
        self.online_eval_tp = []
        self.online_eval_fp = []
        self.online_eval_tn = []
        self.online_eval_fn = []
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            self.online_eval_foreground_acc.append(
                list(
                    (tp_hard + tn_hard) / (tp_hard + fp_hard + tn_hard + fn_hard + 1e-8)
                )
            )
            self.online_eval_foreground_sen.append(
                list((tp_hard) / (tp_hard + fn_hard + 1e-8))
            )
            self.online_eval_foreground_spe.append(
    ...
    def finish_online_evaluation(self):
        ...
        global_sen_per_class = [
            i
            for i in [
                i / (i + j) for i, j in zip(self.online_eval_tp, self.online_eval_fn)
            ]
            if not np.isnan(i)
        ]
        ...
        global_spe_per_class = [
            i
            for i in [
                i / (i + j) for i, j in zip(self.online_eval_tn, self.online_eval_fp)
            ]
            if not np.isnan(i)
        ]

        self.all_val_eval_metrics.append(
            np.mean(global_dc_per_class),
            # np.mean(global_acc_per_class),
            # np.mean(global_sen_per_class),
            # np.mean(global_spe_per_class),
        )

        self.print_to_log_file(
            "Average global foreground Dice: {} Acc: {} Sen: {} Spe: {}".format(
                [np.round(i, 4) for i in global_dc_per_class],
                [np.round(i, 4) for i in global_acc_per_class],
                [np.round(i, 4) for i in global_sen_per_class],
                [np.round(i, 4) for i in global_spe_per_class],
            )
        )
        self.print_to_log_file(
