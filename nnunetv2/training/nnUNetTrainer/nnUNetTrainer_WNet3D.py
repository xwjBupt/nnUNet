...
class ResBlock(nn.Module):
    def __init__(self, inplanes, planes, groups=1):
        ...
        if self.inplanes != self.planes:
            self.down = nn.Sequential(
                nn.Conv3d(inplanes, planes, 1, stride=1), BNNorm3d(planes)
            )
...
class OPE(nn.Module):
    def __init__(self, inplanes, planes, groups=1):
        ...
        self.conv1 = nn.Conv3d(
            inplanes, inplanes, 3, stride=1, groups=groups, padding=1
        )
...
class WNet3D(nn.Module):
    def __init__(
        ...
        self.input_l0 = nn.Sequential(
            ...
            Activation(),
            nn.Conv3d(
                layer_channel[0], layer_channel[0], kernel_size=3, stride=1, padding=1
            ),
            BNNorm3d(layer_channel[0]),
            Activation(),
        )
        ...
        self.encoder2_l1_local = local_block(
            layer_channel[0] + global_dim[0],
            layer_channel[0],
            layer_channel[1],
            down_or_up="down",
        )
        self.encoder2_l1_global = global_block(
            layer_channel[0] + global_dim[0],
            global_dim[0],
            num_heads=num_heads[0],
            sr_ratio=sr_ratio[0],
        ...
        self.encoder2_l2_local = local_block(
            layer_channel[1] + global_dim[1],
            layer_channel[1],
            layer_channel[2],
            down_or_up="down",
        )
        self.encoder2_l2_global = global_block(
            layer_channel[1] + global_dim[1],
            global_dim[1],
            num_heads=num_heads[1],
            sr_ratio=sr_ratio[1],
        ...
        self.encoder2_l3_local = local_block(
            layer_channel[2] + global_dim[2],
            layer_channel[2],
            layer_channel[3],
            down_or_up="down",
        )
        self.encoder2_l3_global = global_block(
            layer_channel[2] + global_dim[2],
            global_dim[2],
            num_heads=num_heads[2],
            sr_ratio=sr_ratio[2],
        ...
        self.encoder2_l4_local = local_block(
            layer_channel[3] + global_dim[3],
            layer_channel[3],
            layer_channel[4],
            down_or_up="down",
        )
        self.encoder2_l4_global = global_block(
            layer_channel[3] + global_dim[3],
            global_dim[3],
            num_heads=num_heads[3],
            sr_ratio=sr_ratio[3],
        ...
        self.decoder2_l4_local_output = nn.Conv3d(
            layer_channel[4], num_classes, kernel_size=1, stride=1, padding=0
        )
        self.decoder2_l4_local = local_block(
            layer_channel[4] + global_dim[4],
            layer_channel[4],
            layer_channel[3],
            down_or_up="up",
        )

        self.decoder2_l3_local_output = nn.Conv3d(
            layer_channel[3], num_classes, kernel_size=1, stride=1, padding=0
        )
        self.decoder2_l3_local = local_block(
            layer_channel[3] + global_dim[3],
            layer_channel[3],
            layer_channel[2],
            down_or_up="up",
        ...
        self.decoder2_l2_local_output = nn.Conv3d(
            layer_channel[2], num_classes, kernel_size=1, stride=1, padding=0
        )
        self.decoder2_l2_local = local_block(
            layer_channel[2] + global_dim[2],
            layer_channel[2],
            layer_channel[1],
            down_or_up="up",
        ...
        self.decoder2_l1_local_output = nn.Conv3d(
            layer_channel[1], num_classes, kernel_size=1, stride=1, padding=0
        )
        self.decoder2_l1_local = local_block(
            layer_channel[1] + global_dim[1],
            layer_channel[1],
            layer_channel[0],
            down_or_up="up",
        ...
        self.output_l0 = nn.Sequential(
            ...
            nn.Conv3d(
                layer_channel[0], num_classes, kernel_size=1, stride=1, padding=0
            ),
        )
